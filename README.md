...
💡 You can use this SQL code to create database tables, which will be done using Supabase in PostgreSQL.

```sql
-- Users Table
CREATE TABLE public.users (
  userid INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  namesurname CHARACTER VARYING(100) NOT NULL,
  email CHARACTER VARYING(150) NOT NULL,
  password CHARACTER VARYING(255) NOT NULL,
  registrationdate TIMESTAMP WITH TIME ZONE NULL DEFAULT NOW(),
...
-- UserCities Table (Relationship between Users and Cities)
CREATE TABLE public.usercities (
  recordid INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  userid INTEGER NULL,
  cityid INTEGER NULL,
  inappnotificationon BOOLEAN NULL DEFAULT true,
  dailyweatheremailson BOOLEAN NULL DEFAULT true,
  importantweatheremailson BOOLEAN NULL DEFAULT true,
  CONSTRAINT usercities_pkey PRIMARY KEY (recordid),
  CONSTRAINT usercities_userid_cityid_key UNIQUE (userid, cityid),
  CONSTRAINT usercities_cityid_fkey FOREIGN KEY (cityid) REFERENCES cities (cityid) ON DELETE CASCADE,
  CONSTRAINT usercities_userid_fkey FOREIGN KEY (userid) REFERENCES users (userid) ON DELETE CASCADE
) TABLESPACE pg_default;

-- Weather Table (Relationship with Cities, JSON data format)
CREATE TABLE public.weather (
  weatherid INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  cityid INTEGER NULL,
  weatherdata jsonb NOT NULL,
  constraint weather_pkey PRIMARY KEY (weatherid),
  constraint weather_cityid_fkey FOREIGN KEY (cityid) REFERENCES cities (cityid) ON DELETE CASCADE
) TABLESPACE pg_default;

-- Notifications Table (Relationship with Users)
CREATE TABLE public.notifications (
  notificationid INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  userid INTEGER NULL,
  notificationmessage TEXT NOT NULL,
  notificationdatetime TIMESTAMP WITH TIME ZONE NULL DEFAULT NOW(),
  notificationtype CHARACTER VARYING(100) NULL,
  CONSTRAINT notifications_pkey PRIMARY KEY (notificationid),
  CONSTRAINT notifications_userid_fkey FOREIGN KEY (userid) REFERENCES users (userid) ON DELETE CASCADE
) TABLESPACE pg_default;

-- IpLocationUser Table
CREATE TABLE public.iplocationusers (
  iplocationid INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  userid INTEGER NOT NULL,
  locationdata JSONB NULL,
  CONSTRAINT IpLocationUser_pkey PRIMARY KEY (iplocationid),
  CONSTRAINT IpLocationUser_userid_fkey FOREIGN KEY (userid) REFERENCES users (userid)
) TABLESPACE pg_default;
```
...
erDiagram
    ...
    USERS ||--o{ USERCITIES : "has"
    USERS ||--o{ IPLOCATIONUSER : "has"
    USERCITIES }o--|| CITIES : "registers"
    CITIES ||--o{ WEATHER : "has"

    USERS {
        INTEGER userid PK
        VARCHAR(100) namesurname
        VARCHAR(150) email
    ...
    NOTIFICATIONS {
        ...
        INTEGER userid FK
        VARCHAR(100) notificationmessage
        TIMESTAMPTZ notificationdatetime
    ...
    USERCITIES {
        ...
        INTEGER userid FK
        INTEGER cityid FK
        BOOLEAN inappnotificationon
        BOOLEAN dailyweatheremailson
        BOOLEAN importantweatheremailson
    }
...
- **Notifications:** Table where notifications sent to users are stored.
  ...
  - `NotificationDateTime` (DATETIME): Notification sending date.
  - `NotificationType` (VARCHAR(100)): Notification type.
